...
if (isset($_POST['submit'])) {
    ...
    $audience = $_POST['audience'];
		...
		$folder_dir = "../uploads/notice_file/";
		...
		$base = basename($_FILES['notice_file']['name']);
		...
		if (file_exists($_FILES['notice_file']['tmp_name'])) {
			...
			if ($resumeFileType == "pdf") {
				...
				if ($_FILES['notice_file']['size'] < 500000) { // File size is less than 5MB

					//If all above condition are met then copy file from server temp location to uploads folder.
					move_uploaded_file($_FILES["notice_file"]["tmp_name"], $filename);
				} else {
					//Size Error
    ...
    $sql = "INSERT INTO notice(subject,notice,audience,notice_file,hash,date) VALUES ('$subject','$notice','$audience','$file', '$hash',now())";
            ...
            <section>
                ...
                <form class="centre " action="" method="POST">
                    ...
                    <div id="file" class="form-group">
                        ...
                        <input type="file" name="notice_file" class="btn btn-flat btn-primary">
                    </div>
                        ...
                        <tbody>
                            ...
                            ?>
                                    ...
                                    <?php if ($row['notice_file'] != '') { ?>
                                        <td><a href="../uploads/resume/<?php echo $row['notice_file']; ?>" download="<?php echo 'Notice'; ?>"><i class="fa fa-file"></i></a></td>
                                    <?php } else { ?>
